...
public class Aplicacao {
	...
	public  static void lerArquivo(Jogo[] vetor) {
		...
		static void mergeSort(Jogo[] vetor, int esq, int dir) throws IOException
			 {
				 int comp = 0;
				 int mov = 0;
				 ...
				 esq=0;
				 dir = vetor.length - 1;
				 if (esq < dir) {int meio = (esq + dir) / 2;
					mergeSort(vetor, esq, meio);
					mergeSort(vetor, meio + 1, dir);
					intercalar(vetor, esq, meio, dir);
			 }
					 long time = System.currentTimeMillis() - start;
					 String nomeArquivoMerge = "730216_merge.txt";
					 ArquivoTextoEscrita arquivoEscritaMerge= new ArquivoTextoEscrita(nomeArquivoMerge);
					 arquivoEscritaMerge.escrever("730216" + "\t" + time + "\t" + comp + "\t" + mov);
					 arquivoEscritaMerge.fecharArquivo();
				 }

				static void intercalar(Jogo[] vetor, int esq, int meio, int dir) {
				 ...
				 public static boolean compara(Jogo vetor, Jogo tempJogo){
					if((vetor.getLocal().compareTo((tempJogo.getSelecao1())))>0){
							return true;
					}else if((vetor.getLocal())==((tempJogo.getLocal())) && ((vetor.getAno()) > ((tempJogo.getAno())))){
							return true;
					}else if((vetor.getLocal())==((tempJogo.getLocal())) && ((vetor.getAno()) == ((tempJogo.getAno()))) && ((vetor.getMes()) > ((tempJogo.getMes())))){
						return true;
					}else if((vetor.getLocal())==((tempJogo.getLocal())) && ((vetor.getAno()) == ((tempJogo.getAno()))) && ((vetor.getMes()) == ((tempJogo.getMes()))) && ((vetor.getDia()) > ((tempJogo.getDia())))){
						return true;
					}else{
							return false;
					}
...
mergeSort(pesquisa,tamanho,quantidade);
printArray(pesquisa);
}
}
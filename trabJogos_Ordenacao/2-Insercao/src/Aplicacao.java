...
public class Aplicacao {
	...
	public  static void lerArquivo(Jogo[] vetor) {
		...
		static void insertionSort(Jogo[] vetor, int n) throws IOException
			 {
				 int comp = 0;
				 int mov = 0;
				 long start = System.currentTimeMillis();
					 for (int i = 1; i < n; i++) {
								...
								mov++;
								int j=i-1;	
								while ((j >= 0) && compara(vetor[j], temp) ) {
									comp++;
									vetor[j + 1] = vetor[j];
									mov++;
									j--;
					 ...
					 String nomeArquivoBolha = "730216_insercao.txt";
				 ...
				 public static boolean compara(Jogo vetor, Jogo tempJogo){
					if((vetor.getAno()) > ((tempJogo.getAno()))){
							return true;
					}else if((vetor.getAno())==((tempJogo.getAno())) && ((vetor.getMes()) > ((tempJogo.getMes())))){
							return true;
					}else if((vetor.getAno())==((tempJogo.getAno())) && ((vetor.getMes()) == ((tempJogo.getMes()))) && ((vetor.getDia()) > ((tempJogo.getDia())))){
						return true;
					}else if((vetor.getAno())==((tempJogo.getAno())) && ((vetor.getMes()) == ((tempJogo.getMes()))) && ((vetor.getDia()) == ((tempJogo.getDia()))) && ((vetor.getSelecao1()).compareTo((tempJogo.getSelecao1())))>0){
						return true;
					}else{
							return false;
					}
...
insertionSort(pesquisa,quantidade);
printArray(pesquisa);
}
}